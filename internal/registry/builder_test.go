...
func TestBuildIndexWithPlugins(t *testing.T) {
	...
	metadataPath := filepath.Join(tempDir, "plugins", "test-plugin", "metadata.json")
...
func TestLoadMetadataWithFile(t *testing.T) {
	...
	pluginMetadataDir := filepath.Join(tempDir, "plugin-dir")
	if err := os.MkdirAll(pluginMetadataDir, 0755); err != nil {
		t.Fatalf("Failed to create plugin metadata dir: %v", err)
	}

	metadataPath := filepath.Join(pluginMetadataDir, "metadata.json")
	if err := os.WriteFile(metadataPath, metadataJSON, 0644); err != nil {
		t.Fatalf("Failed to write metadata file: %v", err)
	}
